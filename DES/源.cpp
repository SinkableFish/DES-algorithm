...
uint64_t des(uint64_t input, uint64_t *key, char mode) {
	...
	C = (uint32_t)((permuted_choice_1 >> 28) & 0x000000000fffffff); //密钥左半部分
	D = (uint32_t)(permuted_choice_1 & 0x000000000fffffff);			//密钥右半部份
	...
	for (i = 0; i< 16; i++) {
		...
		for (j = 0; j < iteration_shift[i]; j++) {
			...
			C = 0x0fffffff & (C << 1) | 0x00000001 & (C >> 27);		//交换左右部分
	...
	*key = 0;                   //通过指针保存已置换过的密钥
	for (i = 0; i < 16; i++)
	{
		*key = sub_key[i]*16 + *key;
	}

	return inv_init_perm_res;   //返回一次加密的密文
...
int main(int argc, const char * argv[]) {
	...
	uint64_t input = 0x502a5f1897ec0000; //输入名字“倪弘韬”的unicode
	uint64_t key = 0x0123456789ABCDEF;	 //密钥
